...
def _seed_experience_from_gflownet_yaml(experience, voc, Prior, config):
    ...
    if resolved is None or not os.path.exists(resolved):
        print(f"[reinvent_gflownet_seeded] seed file not found: {seed_yaml}")
        return 0
    ...
    if not seed_rows:
        print("[reinvent_gflownet_seeded] no valid rows found in seed yaml")
        return 0

    # Keep only molecules parseable by RDKit and encodable by current REINVENT vocab.
    filtered_rows = []
    seen = set()
    ...
    print(
        f"[reinvent_gflownet_seeded] seeded replay with {seeded} molecules from {resolved} "
...
class REINVENT_Optimizer(BaseOptimizer):
    ...
    def __init__(self, args=None):
        super().__init__(args)
        self.model_name = "reinvent_gflownet_seeded"
