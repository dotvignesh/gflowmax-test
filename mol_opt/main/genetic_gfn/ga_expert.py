...
class STONED:
    ...
    def query(self, query_size, mating_pool, pool, rank_coefficient=0.01, mutation_rate=None, return_dist=False):
        if not _STONED_DEPS_AVAILABLE:
            raise ImportError(
                "STONED genetic operator requires SELFIES dependencies. "
                "Install with `pip install selfies` and ensure TDC MolConvert deps are available."
            )
        # print(mating_pool)
        if mutation_rate is None:
            mutation_rate = self.mutation_rate
