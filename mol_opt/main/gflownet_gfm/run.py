...
path_main = os.path.dirname(os.path.dirname(path_here))
if path_here not in sys.path:
...
class Dataset:
    ...
    def __init__(self, config, bpath, device, floatX=torch.double):
        ...
        self.priority_mode = get('priority_mode', 'hybrid')
        self.priority_flow_weight = get('priority_flow_weight', 1.0)
        self.priority_reward_weight = get('priority_reward_weight', 0.25)
        self.priority_reward_exp = get('priority_reward_exp', 2.0)
        self.priority_eps = get('priority_eps', 1e-8)
        self.priority_sampling_alpha = get('priority_sampling_alpha', 0.7)
        self.priority_uniform_mix = get('priority_uniform_mix', 0.05)
        self.priority_warmup_size = get('priority_warmup_size', 64)
        self.priority_clip_quantile = get('priority_clip_quantile', 0.95)

        self.online_mols = []
        self.max_online_mols = 1000
        self._traj_counter = 0
        self._online_mols_lock = threading.Lock()
    ...
    def _get(self, i, dset):
        ...
        if len(m.stems):
            samples.append(((m,), ((-1, 0),), r, m, done, traj_id, step_id))
            step_id += 1
            r = done = 0
        while len(m.blocks): # and go backwards
            parents, actions = zip(*self.mdp.parents(m))
            samples.append((parents, actions, r, m, done, traj_id, step_id))
            step_id += 1
            r = done = 0
    ...
    def _get_sample_model(self):
        ...
        for t in range(max_blocks):
            ...
            if t >= self.min_blocks and action == 0:
                ...
                samples.append(((m,), ((-1,0),), r, m, 1, traj_id, step_id))
                step_id += 1
                break
            else:
                ...
                if len(m.blocks) and not len(m.stems) or t == max_blocks - 1:
                    ...
                    if self.do_wrong_thing:
                        samples.append(((m_old,), (action,), r, m, 1, traj_id, step_id))
                    else:
                        samples.append((*zip(*self.mdp.parents(m)), r, m, 1, traj_id, step_id))
                    step_id += 1
                    break
                else:
                    if self.do_wrong_thing:
                        samples.append(((m_old,), (action,), 0, m, 0, traj_id, step_id))
                    else:
                        samples.append((*zip(*self.mdp.parents(m)), 0, m, 0, traj_id, step_id))
                    step_id += 1
    ...
    def _add_mol_to_online(self, r, m, inflow):
        with self._online_mols_lock:
            if self.replay_mode == 'online':
                ...
                safe_r = max(float(r), self.R_min)
                flow_residual = abs(float(inflow) - np.log(safe_r))
                reward_term = safe_r ** self.priority_reward_exp
                if self.priority_mode == 'flow':
                    priority = flow_residual + self.priority_eps
                elif self.priority_mode == 'reward':
                    priority = self.priority_reward_weight * reward_term + self.priority_eps
                else:
                    priority = (
                        self.priority_flow_weight * flow_residual
                        + self.priority_reward_weight * reward_term
                        + self.priority_eps
                    )
                if not np.isfinite(priority) or priority <= 0:
                    priority = self.priority_eps
                self.online_mols.append((priority, m))
    ...
    def sample(self, n):
        if self.replay_mode == 'dataset': ##### random sampling from all 
            ...
            with self._online_mols_lock:
                online_snapshot = list(self.online_mols)
            eidx = np.random.randint(0, max(1, len(online_snapshot)), n)
            samples = sum((self._get(i, online_snapshot) for i in eidx), [])
        elif self.replay_mode == 'prioritized': #### weight sampling. see _add_mol_to_online above 
            with self._online_mols_lock:
                online_snapshot = list(self.online_mols)
            if not len(online_snapshot):
                # _get will sample from the model
                samples = sum((self._get(0, online_snapshot) for i in range(n)), [])
            else:
                ##### weight sampling 
                prio = np.asarray([i[0] for i in online_snapshot], dtype=np.float64)
                prio = np.where(np.isfinite(prio) & (prio > 0), prio, self.priority_eps)
                if len(online_snapshot) < self.priority_warmup_size:
                    probs = None
                else:
                    if 0.0 < self.priority_clip_quantile < 1.0:
                        clip_val = np.quantile(prio, self.priority_clip_quantile)
                        clip_val = max(float(clip_val), self.priority_eps)
                        prio = np.minimum(prio, clip_val)
                    prio = np.power(prio, self.priority_sampling_alpha)
                    prio_sum = prio.sum()
                    if prio_sum <= 0:
                        probs = None
                    else:
                        probs = prio / prio_sum
                        if self.priority_uniform_mix > 0:
                            probs = ((1.0 - self.priority_uniform_mix) * probs
                                     + self.priority_uniform_mix / len(online_snapshot))
                replace = len(online_snapshot) < n
                eidx = np.random.choice(len(online_snapshot), n, replace=replace, p=probs)
                samples = sum((self._get(i, online_snapshot) for i in eidx), [])
        return zip(*samples)
        # return samples

    def sample2batch(self, mb):
        p, a, r, s, d, traj_id, step_id, *o = mb
        mols = (p, s)
        ...
        traj_id = torch.tensor(traj_id, device=self._device).long()
        step_id = torch.tensor(step_id, device=self._device).long()
        return (p, p_batch, a, r, s, d, mols, traj_id, step_id, *o)
...
class GFlowNet_Optimizer(BaseOptimizer):
    ...
    def _optimize(self, oracle, config):
        ...
        self.oracle.normalize_auc_to_observed = True
        ...
        subtb_weight = float(getattr(config, 'subtb_weight', 0.0))
        subtb_lambda = float(getattr(config, 'subtb_lambda', 0.8))
        subtb_max_span = int(getattr(config, 'subtb_max_span', 4))
        subtb_warmup_calls = int(getattr(config, 'subtb_warmup_calls', 500))
        ####### initialize setup #########
        ...
        while True:
            ...
            if not debug_no_threads:
                ...
                for thread in dataset.sampler_threads:
                    if thread.failed:
                        ...
                        return
                p, pb, a, r, s, d, mols, traj_id, step_id = r
            else:
                p, pb, a, r, s, d, mols, traj_id, step_id = dataset.sample2batch(dataset.sample(mbsize)) #### see dataset.sample 
            ####### 1. data ########
            ...
            if balanced_loss:
                base_loss = term_loss * leaf_coef + flow_loss
            else:
                base_loss = losses.mean()

            delta = inflow - outflow_plus_r
            subtb_loss = delta.new_zeros(())
            if subtb_weight > 0 and len(self.oracle.mol_buffer) >= subtb_warmup_calls:
                total_subtb = delta.new_zeros(())
                n_segments = 0
                for tid in torch.unique(traj_id).tolist():
                    traj_mask = traj_id == tid
                    if traj_mask.sum().item() < 2:
                        continue
                    traj_delta = delta[traj_mask]
                    traj_steps = step_id[traj_mask]
                    order = torch.argsort(traj_steps)
                    traj_delta = traj_delta[order]
                    traj_len = traj_delta.shape[0]
                    max_span_here = min(subtb_max_span, traj_len - 1)
                    if max_span_here < 1:
                        continue
                    for start in range(traj_len):
                        seg_sum = traj_delta[start]
                        for end in range(start + 1, min(traj_len, start + max_span_here + 1)):
                            seg_sum = seg_sum + traj_delta[end]
                            span = end - start
                            total_subtb = total_subtb + (subtb_lambda ** span) * (seg_sum * seg_sum)
                            n_segments += 1
                if n_segments > 0:
                    subtb_loss = total_subtb / n_segments

            loss = base_loss + subtb_weight * subtb_loss
            opt.zero_grad()
            ...
            if len(self.oracle) > 100:
                ...
                if new_scores == old_scores:
                    ...
                    if patience >= self.args.patience * 100:
                        # Log against observed calls on early-stop (no extrapolation to full budget).
...
if __name__ == "__main__":
    raise SystemExit(
        "main/gflownet/run.py is a module, not a standalone entrypoint. "
        "Run from mol_opt with: uv run python run.py --method gflownet --task production "
        "--n_runs 5 --oracles albuterol_similarity --max_oracle_calls 10000 --freq_log 1000 "
        "--log_results"
    )
