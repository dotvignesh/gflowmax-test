...
def _slice_starts(batch, key, device):
    if hasattr(batch, "__slices__") and key in batch.__slices__:
        return torch.tensor(batch.__slices__[key][:-1], dtype=torch.long, device=device)
    if hasattr(batch, "_slice_dict") and key in batch._slice_dict:
        return batch._slice_dict[key][:-1].to(device=device, dtype=torch.long)
    if key == "x" and hasattr(batch, "ptr"):
        return batch.ptr[:-1].to(device=device, dtype=torch.long)
    raise AttributeError(f"Unable to resolve slice starts for key '{key}'")
...
class MPNNet_v2(nn.Module):
    ...
    def forward(self, data, vec_data=None, do_stems=True, do_bonds=False, k=None, do_dropout=False):
        ...
        if do_stems:
            ...
            stem_batch_idx = (
                _slice_starts(data, "x", out.device)[data.stems_batch]
                + data.stems)
...
class MolAC_GCN(nn.Module):
    ...
    def action_negloglikelihood(self, s, a, g, stem_o, mol_o):
        ...
        stem_slices = _slice_starts(s, "stems", stem_lsm.device)
        return -(
    ...
    def index_output_by_action(self, s, stem_o, mol_o, a):
        stem_slices = _slice_starts(s, "stems", stem_o.device)
        return (
